...
const char* error_module( const Utf8::Result::ErrorCode& )
{
    return "Utf8";
};
...
std::vector<unsigned char> Utf8::serialize( const std::u32string &utf8, Error &e )
{
    ...
    for( const char32_t &c : utf8 )
    {
        ...
        {
            e = Utf8::Result::UnsupportedCharacter;
        }
    }
...
std::u32string Utf8::deserialize( const std::vector<unsigned char> &utf8, Error &e )
{
    char32_t c;
    unsigned i = 0;
    std::u32string res;
    ...
    if ( i )
    {
        e = Utf8::Result::UnexpectedEnding;
    }
